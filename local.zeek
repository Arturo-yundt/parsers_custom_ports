...
global tftp_ports_str = getenv("ZEEK_TFTP_PORTS");
global c1222_ports_str = getenv("ZEEK_C1222_PORTS");
global hartip_ports_str = getenv("ZEEK_HARTIP_PORTS");
global roc_plus_ports_str = getenv("ZEEK_ROC_PLUS_PORTS");
global omron_fins_ports_str = getenv("ZEEK_OMRON_FINS_PORTS");
global ge_srtp_ports_str = getenv("ZEEK_GE_SRTP_PORTS");
global opcua_binary_ports_str = getenv("ZEEK_OPCUA_BINARY_PORTS");
global profinet_ports_str = getenv("ZEEK_PROFINET_PORTS");
global tds_ports_str = getenv("ZEEK_TDS_PORTS");
global iec_104_ports_str = getenv("ZEEK_IEC_104_PORTS");
...
event zeek_init() &priority=-5 {
  ...
  if ((!disable_ics_all) && (!disable_ics_opcua_binary) && (opcua_binary_ports_str != "")) {
    local opcua_binary_ports: string_vec = split_string(opcua_binary_ports_str,/,/);
    if (|opcua_binary_ports| > 0) {
      local opcua_binary_ports_tcp: set[port] = {};
      for (opcua_port_idx in opcua_binary_ports) {
        local opcua_binary_port: port = to_port(opcua_binary_ports[opcua_port_idx]);
        local opcua_binary_prot: transport_proto = get_port_transport_proto(opcua_binary_port);
  ...
  if ((!disable_spicy_tftp) && (tftp_ports_str != "")) {
    local tftp_ports = split_string(tftp_ports_str, /,/);
    if (|tftp_ports| > 0) {
      local tftp_ports_udp: set[port] = {};
      for (tftp_port_idx in tftp_ports) {
        local tftp_port = to_port(tftp_ports[tftp_port_idx]);
        local tftp_prot = get_port_transport_proto(tftp_port);
        if (tftp_prot == udp) {
          add tftp_ports_udp[tftp_port];
        }
      }
      if (|tftp_ports_udp| > 0) {
        Analyzer::register_for_ports(Analyzer::ANALYZER_SPICY_TFTP,tftp_ports_udp);
      }
    }
  }

  if ((!disable_ics_all) && (!disable_ics_c1222) && (c1222_ports_str != "")) {
    local c1222_ports: string_vec = split_string(c1222_ports_str,/,/);
    if (|c1222_ports| > 0) {
      local c1222_ports_tcp: set[port] = {};
      local c1222_ports_udp: set[port] = {};
      for (c1222_port_idx in c1222_ports) {
        local c1222_port: port = to_port(c1222_ports[c1222_port_idx]);
        local c1222_prot: transport_proto = get_port_transport_proto(c1222_port);
        if (c1222_prot == tcp) {
          add c1222_ports_tcp[c1222_port];        
        } else if (c1222_prot == udp) {
          add c1222_ports_udp[c1222_port];
        }
      }
      if (|c1222_ports_tcp| > 0) {
        Analyzer::register_for_ports(Analyzer::ANALYZER_C1222_TCP, c1222_ports_tcp);
      }
      if (|c1222_ports_udp| > 0) {
        Analyzer::register_for_ports(Analyzer::ANALYZER_C1222_UDP, c1222_ports_udp);
      }
    }
  }

  if ((!disable_ics_all) && (!disable_ics_hart_ip) && (hartip_ports_str != "")) {
    local hartip_ports: string_vec = split_string(hartip_ports_str, /,/);
    if (|hartip_ports| > 0) {
      local hartip_ports_tcp: set[port] = {};
      local hartip_ports_udp: set[port] = {};
      for (hartip_port_idx in hartip_ports) {
        local hartip_port: port = to_port(hartip_ports[hartip_port_idx]);
        local hartip_prot: transport_proto =
          get_port_transport_proto(hartip_port);
        if (hartip_prot == tcp) { 
  ...
  if ((!disable_ics_all) && (!disable_ics_roc_plus) && (roc_plus_ports_str != "")) {
    local roc_plus_ports: string_vec = split_string(roc_plus_ports_str, /,/);
    if (|roc_plus_ports| > 0) {
      local roc_plus_ports_tcp: set[port] = {};
      local roc_plus_ports_udp: set[port] = {};
      for (roc_plus_port_idx in roc_plus_ports) {
        local roc_plus_port: port =
          to_port(roc_plus_ports[roc_plus_port_idx]);
        local roc_plus_prot: transport_proto =
          get_port_transport_proto(roc_plus_port);
        if (roc_plus_prot == tcp) {
          add roc_plus_ports_tcp[roc_plus_port];
        } else if (roc_plus_prot == udp) {
          add roc_plus_ports_udp[roc_plus_port];
        }
      }
      if (|roc_plus_ports_tcp| > 0) {
        Analyzer::register_for_ports(Analyzer::ANALYZER_ROC_PLUS_TCP,roc_plus_ports_tcp);
      }
      if (|roc_plus_ports_udp| > 0) {
        Analyzer::register_for_ports(Analyzer::ANALYZER_ROC_PLUS_UDP,roc_plus_ports_udp);
      }
    }
  }

  if ((!disable_ics_all) && (!disable_ics_omron_fins) && (omron_fins_ports_str != "")) {
    local omron_fins_ports: string_vec = split_string(omron_fins_ports_str, /,/);
    if (|omron_fins_ports| > 0) {
      local omron_fins_ports_tcp: set[port] = {};
      local omron_fins_ports_udp: set[port] = {};
      for (omron_fins_port_idx in omron_fins_ports) {
        local omron_fins_port: port =
          to_port(omron_fins_ports[omron_fins_port_idx]);
        local omron_fins_prot: transport_proto =
          get_port_transport_proto(omron_fins_port);
        if (omron_fins_prot == tcp) {
          add omron_fins_ports_tcp[omron_fins_port];
        } else if (omron_fins_prot == udp) {
          add omron_fins_ports_udp[omron_fins_port];
        }
      }
      if (|omron_fins_ports_tcp| > 0) {
        Analyzer::register_for_ports(Analyzer::ANALYZER_OMRON_FINS_TCP,omron_fins_ports_tcp);
      }
      if (|omron_fins_ports_udp| > 0) {
        Analyzer::register_for_ports(Analyzer::ANALYZER_OMRON_FINS_UDP,omron_fins_ports_udp);
      }
    }
  }

  if ((!disable_ics_all) && (!disable_ics_ge_srtp) && (ge_srtp_ports_str != "")) {
    local ge_srtp_ports: string_vec = split_string(ge_srtp_ports_str, /,/);
    if (|ge_srtp_ports| > 0) {
      local ge_srtp_ports_tcp: set[port] = {};
      for (ge_srtp_port_idx in ge_srtp_ports) {
        local ge_srtp_port: port =
          to_port(ge_srtp_ports[ge_srtp_port_idx]);
        local ge_srtp_prot: transport_proto =
          get_port_transport_proto(ge_srtp_port);
        if (ge_srtp_prot == tcp) {
          add ge_srtp_ports_tcp[ge_srtp_port];
        }
      }
      if (|ge_srtp_ports_tcp| > 0) {
        Analyzer::register_for_ports(Analyzer::ANALYZER_SPICY_GE_SRTP_TCP,ge_srtp_ports_tcp);
      }
    }
  } 

  if ((!disable_ics_all) && (!disable_ics_profinet) && (profinet_ports_str != "")) {
    local profinet_ports: string_vec = split_string(profinet_ports_str, /,/);
    if (|profinet_ports| > 0) {
      local profinet_ports_udp: set[port] = {};
      for (profinet_port_idx in profinet_ports) {
        local profinet_port: port =
          to_port(profinet_ports[profinet_port_idx]);
        local profinet_prot: transport_proto =
          get_port_transport_proto(profinet_port);
        if (profinet_prot == udp) {
          add profinet_ports_udp[profinet_port];
        }
      }
      if (|profinet_ports_udp| > 0) {
        Analyzer::register_for_ports(Analyzer::ANALYZER_PROFINET,profinet_ports_udp);
      }
    }
  }

  if ((!disable_ics_all) && (tds_ports_str != "")) {
    local tds_ports: string_vec = split_string(tds_ports_str, /,/);
    if (|tds_ports| > 0) {
      local tds_ports_tcp: set[port] = {};
      for (tds_port_idx in tds_ports) {
        local tds_port: port =
          to_port(tds_ports[tds_port_idx]);
        local tds_prot: transport_proto =
          get_port_transport_proto(tds_port);
        if (tds_prot == tcp) {
          add tds_ports_tcp[tds_port];
        }
      }
      if (|tds_ports_tcp| > 0) {
        Analyzer::register_for_ports(Analyzer::ANALYZER_TDS,tds_ports_tcp);
      }
    }
  }

  if ((!disable_ics_all) && (!disable_ics_iec104) && (iec_104_ports_str != "")) {
    local iec_104_ports: string_vec = split_string(iec_104_ports_str, /,/);
    if (|iec_104_ports| > 0) {
      local iec_104_ports_tcp: set[port] = {};
      for (iec_104_port_idx in iec_104_ports) {
        local iec_104_port: port =
          to_port(iec_104_ports[iec_104_port_idx]);
        local iec_104_prot: transport_proto =
          get_port_transport_proto(iec_104_port);
        if (iec_104_prot == tcp) {
          add iec_104_ports_tcp[iec_104_port];
        }
      }
      if (|iec_104_ports_tcp| > 0) {
        Analyzer::register_for_ports(Analyzer::ANALYZER_SPICY_IEC104,iec_104_ports_tcp);
      }
    }
  }
}
